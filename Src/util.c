...
// APP扩展命令全局状态
volatile uint16_t serial_buttons = 0;
volatile uint8_t  serial_buttons_updated = 0;
...
/* =========================== BBCAR功能函数开始 =========================== */
  ...
  #ifdef VARIANT_BBCAR
    static float speedRL = 0.0;  // 高精度内部速度计算值 [-1000.0 to 1000.0]
    static float weak = 0.0;     // 磁场削弱强度 [-0.0 to 500.0]
    
    extern int16_t speedL, speedR, speed;  // 外部声明的速度变量

    static float acc_cmd = 0.0;  // 加速踏板命令 (0.0 - 1.0)
    static float brk_cmd = 0.0;  // 刹车踏板命令 (0.0 - 1.0)
    
    static float input1_filtered;  // 输入1滤波后的值 (ADC1 - 右踏板/油门/蓝色)
    static float input2_filtered;  // 输入2滤波后的值 (ADC2 - 左踏板/刹车/绿色)

    extern int8_t drive_mode;  // 驾驶模式 (1-4)

    /**
     * @brief ADC故障提示音
     * @note  当电位器超出范围时播放急促的警告音
     *        发出"滴-滴-滴"的急促声音提示用户校准
     */
     ...
     * @param minn: 最小值
     * @param maxx: 最大值
     * @return 1: 在最小值附近, 0: 不在
     * @note  用于开机时检测踏板位置以选择驾驶模式
     *        考虑到ADC_MARGIN的影响，使用实际ADC值进行判断
     */
    int32_t isAroundMin(int32_t value, int32_t minn, int32_t maxx){
      // 最小值和最大值校准值已经包含了ADC_MARGIN的加减
      // 这对于计算油门范围很有用，但对于检测驾驶模式不适用
      // 所以这里我们使用实际的ADC值：
      minn -= ADC_MARGIN;
      maxx += ADC_MARGIN;

      int32_t delta = ((maxx - minn) * 15) / 100;  // 范围*0.15作为容差
     ...
     * @param minn: 最小值
     * @param maxx: 最大值
     * @return 1: 在最大值附近, 0: 不在
     * @note  用于开机时检测踏板是否被完全踩下
     */
    ...
    /**
     * @brief 开机驾驶模式检测
     * @note  开机时根据踏板位置选择驾驶模式：
     *        模式1 (儿童模式): 左踏板踩下, 右踏板松开 → 最高约3km/h, 无涡轮
     *        模式2 (标准模式): 双踏板松开 → 最高约6km/h, 无涡轮 (默认)
     *        模式3 (乐趣模式): 右踏板踩下, 左踏板松开 → 最高约12km/h, 无涡轮
     *        模式4 (动力模式): 双踏板踩下 → 最高约22km/h, 涡轮约39km/h
     *        
     *        流程: 踩住踏板 → 开机 → 听到模式提示音 → 松开踏板
     */
...
/* =========================== DIY卡丁车固件 v17.03 功能实现 =========================== */
    ...
    // ==========================================
      ...
      static uint8_t uart_connected = 0;
      static int16_t uart_cmdL = 0, uart_cmdR = 0;
      ...
      void dualControlProcess(int16_t adc_cmdL, int16_t adc_cmdR, int16_t *cmdL, int16_t *cmdR) {
        // 如果UART有输入且有效，优先使用UART
        if (uart_connected) {
          if (uart_cmdL != 0 || uart_cmdR != 0) {
            *cmdL = uart_cmdL;
            *cmdR = uart_cmdR;
            return;
          }
    ...
    // ==========================================
      ...
      static uint8_t cruise_enabled = 0;
      static uint8_t cruise_active = 0;
      static int16_t cruise_target_speed = 0;
      static uint32_t cruise_last_cmd_time = 0;
      
      void cruiseControlInit(void) {
        cruise_enabled = 0;
        cruise_active = 0;
        cruise_target_speed = 0;
        cruise_last_cmd_time = 0;
      }
      
      void cruiseControlEnable(uint8_t enable) {
        if (enable && !cruise_enabled) {
          cruise_enabled = 1;
          bbcarBeep(12);  // 巡航开启提示音
        } else if (!enable && cruise_enabled) {
          cruise_enabled = 0;
          cruise_active = 0;
          cruise_target_speed = 0;
          bbcarBeep(13);  // 巡航关闭提示音
        }
      }
      ...
      void cruiseControlSetSpeed(int16_t speed) {
        if (speed >= CRUISE_MIN_SPEED && speed <= CRUISE_MAX_SPEED) {
          cruise_target_speed = speed;
          cruise_active = 1;
        }
      }
      ...
      int16_t cruiseControlProcess(int16_t cmd, int16_t speedAvg) {
        ...
        // 如果有新的输入命令，关闭巡航
        if (cmd != 0) {
          cruise_last_cmd_time = HAL_GetTick();
          if (cruise_active) {
            cruise_active = 0;
          }
          return cmd;
        ...
        if (cruise_active) {
          // 简单的速度PI控制
          int16_t speed_error = cruise_target_speed - ABS(speedAvg);
          int16_t cruise_cmd = cmd;
        ...
        // 如果速度足够快且没有输入，激活巡航
        if (ABS(speedAvg) >= CRUISE_MIN_SPEED && cmd == 0) {
          if (HAL_GetTick() - cruise_last_cmd_time > 1000) {
            cruiseControlSetSpeed(ABS(speedAvg));
          }
        }
    ...
    // ==========================================
      ...
      static uint8_t smart_neutral_active = 0;
      static uint32_t neutral_stop_time = 0;
      
      void smartNeutralInit(void) {
        smart_neutral_active = 0;
        neutral_stop_time = 0;
      }
      
      void smartNeutralProcess(void) {
        if (speedAvgAbs < NEUTRAL_STOPPED_THRESHOLD) {
          // 车辆停止
          if (neutral_stop_time == 0) {
            neutral_stop_time = HAL_GetTick();
          }
          // 等待超时后进入空挡
          if (HAL_GetTick() - neutral_stop_time > NEUTRAL_TIMEOUT_MS) {
            smart_neutral_active = 1;
          }
        } else {
          // 车辆运动，重置计时器
          neutral_stop_time = 0;
    ...
    // ==========================================
      ...
      static uint8_t epb_active = 0;
      static uint8_t epb_locked = 0;
      ...
      void epbEnable(uint8_t enable) {
        if (enable && !epb_active) {
          epb_active = 1;
          bbcarBeep(11);  // 手刹提示音
        } else if (!enable && epb_active) {
          epb_active = 0;
          epb_locked = 0;
        }
      }
      ...
      int16_t epbProcess(int16_t cmd, int16_t speed) {
        ...
        if (speedAvgAbs < BRAKE_LOCK_THRESHOLD) {
          epb_locked = 1;
        }
    ...
    // ==========================================
      ...
      static uint8_t hall_cal_state = HALL_CAL_IDLE;
      static uint32_t hall_cal_start_time = 0;
      static int16_t hall_cal_speed = 0;
      static uint8_t hall_cal_phase = 0;
      
      void autoHallCalibrationStart(void) {
        hall_cal_state = HALL_CAL_START;
        hall_cal_start_time = HAL_GetTick();
        bbcarBeep(4);  // 开始校准提示音
      }
      
      uint8_t autoHallCalibrationGetState(void) {
        return hall_cal_state;
      }
      
      void autoHallCalibrationProcess(void) {
        switch (hall_cal_state) {
          case HALL_CAL_START:
            // 启动校准，开始转动电机
            hall_cal_state = HALL_CAL_MEASURING;
            hall_cal_speed = HALL_CALIBRATION_SPEED;
            hall_cal_phase = 0;
            break;
            
          case HALL_CAL_MEASURING:
            // 检测霍尔信号，确定最佳相序
            // 这里需要读取实际的霍尔传感器信号
            // 由于不同硬件实现不同，这里给出一个框架
            if (HAL_GetTick() - hall_cal_start_time > HALL_CALIBRATION_TIMEOUT) {
              hall_cal_state = HALL_CAL_ERROR;
              // 校准失败
            }
            // 实际实现需要读取霍尔传感器并确定最佳相序
            // 这通常需要分析三个霍尔传感器的相位关系
            hall_cal_phase++;
            if (hall_cal_phase >= 6) {
              hall_cal_state = HALL_CAL_DONE;
              hall_cal_speed = 0;
              bbcarBeep(5);  // 校准完成提示音
            }
            break;
    ...
    // ==========================================
    #ifdef BOOST_ENABLE
      static uint8_t boost_level = 0;
      
      int16_t boostProcess(int16_t cmd, int16_t current) {
        if (cmd <= 0) {
          return cmd;  // 只有正输入（前进）才增压
        }
        
        // 根据踏板行程确定增压档位
        float cmd_ratio = (float)ABS(cmd) / 1000.0f;
        
        if (cmd_ratio >= BOOST_THRESHOLD_4) {
          boost_level = 5;
        } else if (cmd_ratio >= BOOST_THRESHOLD_3) {
          boost_level = 4;
        } else if (cmd_ratio >= BOOST_THRESHOLD_2) {
          boost_level = 3;
        } else if (cmd_ratio >= BOOST_THRESHOLD_1) {
          boost_level = 2;
        } else {
          boost_level = 1;
        }
        ...
        // 线性匹配增压强度
        // 实际电流控制需要根据当前电流和目标电流调整
        int16_t boost_current = (boost_level * BOOST_MAX_CURRENT) / 5;
    ...
    // ==========================================
      ...
      static int16_t last_cmd = 0;
      
      int16_t autoDecelProcess(int16_t cmd) {
        #ifdef AUTO_DECEL_COEF
          #if AUTO_DECEL_COEF > 0
            if (cmd == 0 && last_cmd != 0) {
              // 减速优化：当松开踏板时，添加柔和的减速
              int16_t decel_cmd = 0;
              #if AUTO_DECEL_COEF <= 50
                decel_cmd = last_cmd * (50 - AUTO_DECEL_COEF) / 100;
              #endif
              last_cmd = decel_cmd;
              return decel_cmd;
            }
    ...
    // ==========================================
      ...
      typedef struct {
        ...
        uint8_t voltage;          // 电压 (V)
        uint8_t current;          // 电流 (A)
      ...
      static YxtPacket yxt_packet;
      static uint32_t yxt_last_update = 0;
      static uint8_t yxt_fault_status = 0;
      
      /**
       * @brief 初始化一线通仪表通信
       * @note  初始化数据包结构，设置默认显示值
       */
      void yxtDisplayInit(void) {
        yxt_packet.header = 0xAA;       // 一线通协议帧头
        yxt_packet.speed = 0;
        yxt_packet.voltage = 0;
        yxt_packet.current = 0;
        yxt_packet.temperature = 0;
        yxt_packet.gear = 1;            // 默认1档
        yxt_packet.fault = 0;
        yxt_packet.checksum = 0;
        yxt_fault_status = 0;
        yxt_last_update = 0;
      }
      ...
      /**
       * @brief 设置仪表显示速度
       * @param speed: 速度值 (rpm，内部转换为km/h)
       */
      void yxtSetSpeed(int16_t speed) {
        // 将电机转速转换为km/h显示
        // 转换公式: speed(rpm) * 轮子周长(m) * 60 / 1000
        // 简化: 假设轮子周长约0.5m，speed * 0.03 ≈ km/h
        int16_t kmh = ABS(speed) * 3 / 100;
        if (kmh > 255) kmh = 255;
        yxt_packet.speed = (uint8_t)kmh;
      }
      
      /**
       * @brief 设置仪表显示电压
       * @param voltage: 电压值 (单位: 0.1V)
       */
      void yxtSetVoltage(int16_t voltage) {
        // 电压值转换为V (输入是0.1V单位)
        yxt_packet.voltage = (uint8_t)(voltage / 10);
      }
      
      /**
       * @brief 设置仪表显示电流
       * @param current: 电流值 (单位: A)
       */
      void yxtSetCurrent(int16_t current) {
        if (current > 255) current = 255;
        yxt_packet.current = (uint8_t)current;
      }
      
      /**
       * @brief 设置仪表显示温度
       * @param temp: 温度值 (单位: °C * 10)
       */
      void yxtSetTemperature(int16_t temp) {
        yxt_packet.temperature = (uint8_t)(temp / 10);
      }
      ...
      void yxtSetGear(uint8_t gear) {
        if (gear > 5) gear = 5;
        if (gear < 1) gear = 1;
        yxt_packet.gear = gear;
      }
      
      /**
       * @brief 设置仪表故障灯
       * @param fault: 故障码 (YXT_FAULT_xxx)
       */
      void yxtSetFault(uint8_t fault) {
        yxt_fault_status |= fault;
        yxt_packet.fault = yxt_fault_status;
      }
       ...
       * @return 校验和字节
       */
      static uint8_t yxtCalcChecksum(void) {
        uint8_t sum = 0;
        uint8_t *data = (uint8_t *)&yxt_packet;
        for (int i = 0; i < sizeof(YxtPacket) - 1; i++) {
          sum += data[i];
        }
        return sum;
      }
      ...
      void yxtSendPacket(void) {
        yxt_packet.checksum = yxtCalcChecksum();
        
        // 通过UART3发送数据包 (假设使用USART3)
        #ifdef YXT_USART
          HAL_UART_Transmit(&huart3, (uint8_t *)&yxt_packet, sizeof(YxtPacket), 100);
        #endif
      }
      
      /**
       * @brief 更新一线通仪表显示
       * @note  定期调用此函数更新仪表显示内容
       *        根据当前驾驶模式设置对应的故障灯显示
       */
      void yxtDisplayUpdate(void) {
        if (HAL_GetTick() - yxt_last_update < YXT_UPDATE_INTERVAL) {
          return;  // 还未到更新时间
        }
        yxt_last_update = HAL_GetTick();
        
        // 更新速度显示
        #ifdef YXT_SHOW_SPEED
          yxtSetSpeed(speedAvg);
        #endif
        
        // 更新电压显示
        #ifdef YXT_SHOW_VOLTAGE
          yxtSetVoltage(batVoltageCalib);
        #endif
        
        // 更新电流显示
        #ifdef YXT_SHOW_CURRENT
          yxtSetCurrent(dc_curr);
        #endif
        
        // 更新温度显示
        #ifdef YXT_SHOW_TEMP
          yxtSetTemperature(board_temp_deg_c);
        #endif
        
        // 更新档位显示
        #ifdef YXT_SHOW_GEAR
          yxtSetGear((uint8_t)drive_mode);
        #endif
        ...
        // 轿车模式: 亮「转把故障」灯
        // 遥控模式: 亮「控制器故障」灯
        #ifdef YXT_SHOW_ERROR
          yxtClearFault(YXT_FAULT_THROTTLE | YXT_FAULT_CTRL);
          
          #if BBCAR_DRIVE_MODE == BBCAR_MODE_STEER
            // 轿车模式 - 亮转把故障灯
            yxtSetFault(YXT_FAULT_THROTTLE);
          #elif BBCAR_DRIVE_MODE == BBCAR_MODE_RC
            // 遥控模式 - 亮控制器故障灯
            yxtSetFault(YXT_FAULT_CTRL);
          #endif
          
          // 手刹状态 - 刹车灯
          #ifdef ELECTRONIC_PARKING_BRAKE_ENABLE
            if (epbGetState()) {
              yxtSetFault(YXT_FAULT_BRAKE);
            } else {
              yxtClearFault(YXT_FAULT_BRAKE);
            }
          #endif
          
          // 电机故障检测
          if (rtY_Left.z_errCode || rtY_Right.z_errCode) {
            yxtSetFault(YXT_FAULT_MOTOR);
          } else {
            yxtClearFault(YXT_FAULT_MOTOR);
          }
        #endif
    ...
    /**
     * @brief 蜂鸣器提示音类型枚举
     * @note  定义所有可能的蜂鸣器提示音类型
     */
    typedef enum {
      BEEP_TYPE_NONE = 0,           // 无提示音
      BEEP_TYPE_UART_CONNECT,       // 遥控器连接成功
      BEEP_TYPE_MODE_SWITCH,        // 切换驾驶模式
      BEEP_TYPE_CALIB_START,        // 开始自动校准
      BEEP_TYPE_CALIB_DONE,         // 校准完成
      BEEP_TYPE_GEAR_SHIFT,         // 换档
      BEEP_TYPE_HAND_BRAKE,         // 手刹
      BEEP_TYPE_CRUISE_ON,          // 定速巡航开启
      BEEP_TYPE_CRUISE_OFF,         // 定速巡航关闭
      BEEP_TYPE_REVERSE,            // 倒挡
      BEEP_TYPE_HEADLIGHT,          // 大灯
      BEEP_TYPE_LOW_BATTERY,        // 低电量警告
      BEEP_TYPE_OVERHEAT,           // 过热警告
      BEEP_TYPE_HALL_ERROR,         // 霍尔故障
      BEEP_TYPE_POWER_ON,           // 开机
      BEEP_TYPE_POWER_OFF,          // 关机
      BEEP_TYPE_ADC_ERROR,          // ADC故障
      BEEP_TYPE_MAX
    } BeepType;
    ...
    typedef struct {
      uint8_t count;        // 蜂鸣次数
      uint8_t freq;         // 蜂鸣频率
      uint16_t duration;    // 单次持续时间(ms)
      uint16_t interval;    // 间隔时间(ms)
    } BeepConfig;
    
    // 蜂鸣器提示音配置表
    static const BeepConfig beep_configs[] = {
      {0, 0, 0, 0},           // BEEP_TYPE_NONE
      {1, 4, 100, 100},       // BEEP_TYPE_UART_CONNECT - 1声短促
      {2, 4, 100, 200},       // BEEP_TYPE_MODE_SWITCH - 2声
      {4, 5, 80, 100},        // BEEP_TYPE_CALIB_START - 4声快速
      {5, 6, 100, 150},       // BEEP_TYPE_CALIB_DONE - 5声
      {1, 3, 80, 50},         // BEEP_TYPE_GEAR_SHIFT - 1声短促
      {1, 2, 200, 100},       // BEEP_TYPE_HAND_BRAKE - 1声较长
      {1, 5, 100, 100},       // BEEP_TYPE_CRUISE_ON - 1声中频
      {2, 3, 150, 200},       // BEEP_TYPE_CRUISE_OFF - 2声低频
      {1, 2, 100, 100},       // BEEP_TYPE_REVERSE - 1声低频
      {1, 6, 80, 50},         // BEEP_TYPE_HEADLIGHT - 1声高频短促
      {3, 2, 200, 300},       // BEEP_TYPE_LOW_BATTERY - 3声低频长音
      {4, 2, 150, 200},       // BEEP_TYPE_OVERHEAT - 4声低频
      {6, 1, 100, 100},       // BEEP_TYPE_HALL_ERROR - 6声急促(滴...滴...滴...)
      {2, 5, 150, 200},       // BEEP_TYPE_POWER_ON - 开机旋律
      {3, 3, 200, 150},       // BEEP_TYPE_POWER_OFF - 关机旋律
      {6, 6, 50, 50}          // BEEP_TYPE_ADC_ERROR - 6声高频急促
    };
    
    /**
     * @brief 播放指定类型的蜂鸣器提示音
     * @param type: 蜂鸣器类型 (BeepType枚举)
     * @note  根据配置表播放对应的提示音
     */
    void bbcarBeep(uint8_t type) {
      if (type >= BEEP_TYPE_MAX) return;
      
      const BeepConfig *config = &beep_configs[type];
      if (config->count == 0) return;
      
      for (uint8_t i = 0; i < config->count; i++) {
        buzzerFreq = config->freq;
        HAL_Delay(config->duration);
        buzzerFreq = 0;
        if (i < config->count - 1) {
     ...
     * @note  当电池电压低于警告阈值时调用
     */
    void bbcarBeepLowBattery(void) {
      bbcarBeep(BEEP_TYPE_LOW_BATTERY);
    }
     ...
     * @note  当主板温度超过警告阈值时调用
     */
    void bbcarBeepOverheat(void) {
      bbcarBeep(BEEP_TYPE_OVERHEAT);
    }
     ...
     *        发出"滴...滴...滴..."的急促声音
     */
    void bbcarBeepHallError(void) {
      bbcarBeep(BEEP_TYPE_HALL_ERROR);
    }
     ...
     * @note  开机时播放欢迎旋律
     */
    void bbcarBeepPowerOn(void) {
      bbcarBeep(BEEP_TYPE_POWER_ON);
    }
     ...
     * @note  关机时播放告别旋律
     */
    void bbcarBeepPowerOff(void) {
      bbcarBeep(BEEP_TYPE_POWER_OFF);
    }

  #endif

/* =========================== END DIY卡丁车固件 v17.03 功能 =========================== */
...
void usart_process_command(SerialCommand *command_in, SerialCommand *command_out, uint8_t usart_idx)
{
  ...
  if (command_in->start == SERIAL_START_FRAME) {
    checksum = (uint16_t)(command_in->start ^ command_in->steer ^ command_in->speed ^ command_in->buttons);
    if (command_in->checksum == checksum) {
      *command_out = *command_in;
      // 保存APP扩展buttons状态
      serial_buttons = command_in->buttons;
      serial_buttons_updated = 1;
      if (usart_idx == 2) {             // Sideboard USART2
        #ifdef CONTROL_SERIAL_USART2
        timeoutFlgSerial_L = 0;         // Clear timeout flag
        timeoutCntSerial_L = 0;         // Reset timeout counter
