  ...
  while(1) {
    ...
    calcAvgSpeed();                       // Calculate average measured speed: speedAvg, speedAvgAbs
      ...
      if (inIdx == CONTROL_ADC) {               // Only use use implementation below if pedals are in use (ADC input)
        ...
        // DIY卡丁车固件 v17.03 功能处理
        
        // APP扩展命令处理 (SerialCommand.buttons)
        if (serial_buttons_updated) {
          serial_buttons_updated = 0;
          uint16_t btns = serial_buttons;
          
          // 驾驶模式切换
          #ifdef MULTI_MODE_DRIVE
            if (btns & BTN_MODE1)      { drive_mode = 1; }
            else if (btns & BTN_MODE2) { drive_mode = 2; }
            else if (btns & BTN_MODE3) { drive_mode = 3; }
            else if (btns & BTN_MODE4) { drive_mode = 4; }
          #endif
          
          // 电子手刹
          #ifdef ELECTRONIC_PARKING_BRAKE_ENABLE
            epbEnable((btns & BTN_EPB) ? 1 : 0);
          #endif
          
          // 定速巡航
          #ifdef CRUISE_CONTROL_ENABLE
            cruiseControlEnable((btns & BTN_CRUISE) ? 1 : 0);
          #endif
        ...
        // 电子手刹处理
        #ifdef ELECTRONIC_PARKING_BRAKE_ENABLE
          cmdR = cmdL = epbProcess(cmdR, speedAvg);
        #endif
        
        // 定速巡航处理
        #ifdef CRUISE_CONTROL_ENABLE
          cmdR = cmdL = cruiseControlProcess(cmdR, speedAvg);
        #endif
        
        // 增压优化处理
        #ifdef BOOST_ENABLE
          cmdR = boostProcess(cmdR, dc_curr);
          cmdL = boostProcess(cmdL, dc_curr);
        #endif
        
        // 自动减速优化
        #ifdef AUTO_DECEL_ENABLE
          cmdR = autoDecelProcess(cmdR);
          cmdL = autoDecelProcess(cmdL);
        #endif
